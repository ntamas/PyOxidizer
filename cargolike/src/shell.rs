...
impl Shell {
    ...
    pub fn status<T, U>(&mut self, status: T, message: U) -> ShellResult<()>
    where
        T: fmt::Display,
        U: fmt::Display,
    {
    ...
    pub fn status_header<T>(&mut self, status: T) -> ShellResult<()>
    where
        T: fmt::Display,
    {
    ...
    pub fn verbose<F>(&mut self, mut callback: F) -> ShellResult<()>
    where
        F: FnMut(&mut Shell) -> ShellResult<()>,
    {
        match self.verbosity {
    ...
    pub fn concise<F>(&mut self, mut callback: F) -> ShellResult<()>
    where
        F: FnMut(&mut Shell) -> ShellResult<()>,
    {
        match self.verbosity {
    ...
    pub fn error<T: fmt::Display>(&mut self, message: T) -> ShellResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
        self.output
    ...
    pub fn warn<T: fmt::Display>(&mut self, message: T) -> ShellResult<()> {
        match self.verbosity {
            Verbosity::Quiet => Ok(()),
    ...
    pub fn note<T: fmt::Display>(&mut self, message: T) -> ShellResult<()> {
    ...
    pub fn set_color_choice(&mut self, color: Option<&str>) -> ShellResult<()> {
    ...
    pub fn set_unicode(&mut self, yes: bool) -> ShellResult<()> {
        if let ShellOut::Stream {
            stdout_unicode,
            stderr_unicode,
            ..
    ...
    pub fn set_hyperlinks(&mut self, yes: bool) -> ShellResult<()> {
    ...
    fn file_hyperlink(&mut self, path: &std::path::Path) -> Option<url::Url> {
        ...
        let hostname = if cfg!(windows) {
            ...
            if let Some(hostname) = self.hostname.as_deref() {
                ...
                self.hostname = hostname::get().ok().and_then(|h| h.into_string().ok());
    ...
    pub fn print_ansi_stderr(&mut self, message: &[u8]) -> ShellResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
    ...
    pub fn print_ansi_stdout(&mut self, message: &[u8]) -> ShellResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
    ...
    #[cfg(feature = "json")]
    pub fn print_json<T: serde::ser::Serialize>(&mut self, obj: &T) -> ShellResult<()> {
...
enum ShellOut {
    ...
    Write(AutoStream<Box<dyn Write + Send>>),
...
mod tests {
    ...
    impl Display for Change {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            ...
            match self {
                Change::Added(s) => write!(f, "{style}+{style:#} {}", s),
                Change::Removed(s) => write!(f, "{style}-{style:#} {}", s),
            }
        }
    }
    
    #[test]
    fn test_something() -> ShellResult<()> {
        let mut shell = Shell::new();

        shell.status("Updating", "crates.io index")?;

        shell.status("Adding", "anstream v0.6.18 to dependencies")?;
        shell.status("", "Features:")?;
        let features: Vec<Change> = vec![Change::Added("auto".to_string()), Change::Added("wincon".to_string()), Change::Removed("test".to_string())];
        for feature in features {
            shell.status("", feature)?;
        }

        shell.status("Adding", "anstyle v1.0.10 to dependencies")?;
        shell.status("", "Features:")?;
        let features: Vec<Change> = vec![Change::Added("std".to_string())];
        for feature in features {
            shell.status("", feature)?;
        }

        shell.status("Updating", "crates.io index")?;
        shell.status("Locking", "18 packages to latest Rust 1.86.0-nightly compatible version")?;
