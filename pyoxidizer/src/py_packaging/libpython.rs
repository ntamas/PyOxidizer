...
fn create_ar_symbols_index(dest_dir: &Path, lib_data: &[u8]) -> Result<Vec<u8>> {
    ...
    with_verbose_shell(|log| log.status("Creating", "archive symbol index"))?;
...
pub fn link_libpython(
    ...
    with_shell(|log| {
        log.status(
            "Compiling",
            format!(
                "config.c (from {} extension modules)",
    ...
    with_verbose_shell(|log| log.status("Resolving", "inputs for custom Python library"))?;
    ...
    with_shell(|log| {
        log.status("Linking", "customized Python library")
    })?;
    ...
    with_shell(|log| {
        log.status(
            "Finished",
            format!("Python library ({} bytes)", libpython_data.len()),
        )
    })?;
